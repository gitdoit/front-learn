...
> 我他妈真是服了前端这么多配置，一个代码语法、风格、格式化能有这么多配置？？？
>
> [中文官方文档](https://cn.eslint.org/docs/user-guide/configuring)
>
> [vscode中使用eslint](https://zhuanlan.zhihu.com/p/159426292)
>
> [vscode中eslint插件首页](https://marketplace.visualstudio.com/items?itemName=dbaeumer.vscode-eslint&ssr=false#overview)
>
> [vscode中格式化失效](https://zhuanlan.zhihu.com/p/103492877)
>
> 
...
​	据我所知，之前`eslint`只用来做代码质量和编码风格检查，后来又实现了代码格式化，不符合指定规则的在保存的时候直接帮你格式化掉。当然这需要编辑器的规则，在`vscode`里面安装`eslint`插件就行了。



## 2、上手使用

 1. 初始化一个`npm`项目，根目录下创建`.eslint.js`当然可以用其他方式(选择太多，增加心智负担)。
...
> 原谅我的见识浅薄，可能他们这么做是为了兼容各种项目各种环境。但是学起来简直卧槽
...
​	一个项目里面可以允许有多个`eslint`的配置文件，他们按照配置文件的类型和位置，有着不同的优先级。对于一个需要检验的文件，`eslint`首先会找离他最近的配置文件，怎么找呢？先在同级目录下找，然后向上，向上，向上，然后将他们合并，规则冲突了按照`css`那种方式来解决。

当然如果在某个配置文件中出现了`root:true`，那么在读取到这个配置文件之后就会不往后找了。到此为止！
...
## 4、在`vscode`中使用`eslint`插件

> 这个是最坑爹的

### 1、过时的配置

​	不要在网上找各种配置，直接到插件首页看，网上的各种配置要么是过时的，要么是错的。

### 2、不要和`prettier`一起使用

​	之前用这个东西是因为`eslint`不支持格式化，只会给你抛出错误信息，你要手动去改，所以安装这个插件是为了方便格式化，但是现在`eslint`已经支持格式化了，就别一起用了，不然会打架！！！

### 3、父子项目结构

​	一个文件夹下多个项目，每个项目都有自己的`eslint`配置，这时候就比较坑爹了。首先`eslint`插件时借助我们`npm`安装的`eslint`插件来工作的，不同的项目使用不同的`eslint`配置，而且配置中使用的规则集合或者插件都在自己的`workspace`里面，没有全局安装。这个时候需要在`vscode`的`settings.json`里面配置各个子项目的`wordspace`了\

**在当前项目中配置`settings.json`**

```json
{
	"eslint.workingDirectories": [
    // 多项目的时候，每个项目都有一个eslint配置
    // A项目用xx依赖，B项目用DD依赖
    // 每个项目依赖不同，不指定的话eslint在运行的时候，读取各自的配置文件
    // 会找不到配置文件中需要的东西，这里需要指定一下工作目录
		"./3-前端框架/vue3/1-calculator",
		"./5-webpack/1-webpack-base",
		"./5-webpack/2-webpack-optimization",
		"./5-webpack./3-lazy-loading",
		"./5-webpack/4-pwa",
		"./5-webpack/5-externals"
	],
	"eslint.options": {
		// "configFile": "./3-前端框架/vue3/1-calculator/.eslintrc.js",
},
}
...
```json
{
  // 每次保存的时候将代码按eslint格式进行修复，之前是单独的配置，现在统一各个插件了
	"editor.codeActionsOnSave": {
		// 保存时启动eslint
		"source.fixAll.eslint": true
    // 下面这个配置不仅会启动eslint，你安装了prettier也会启动，当然不行
    //"source.fixAll":true
	},
	// 关闭vscode自带的保存格式化，使用上面的配置，指定用eslint来处理；否则会格式化两次
	"editor.formatOnSave": false,
	// 自带的格式化，和eslint冲突，所以需要关闭默认js格式化程序 
	"javascript.format.enable":false,

	/* ***********************ESLINT配置***************************** */
	// eslint之前只用来做语法检查，代码格式检查；现在也能够做代码格式化了
  // https://github.com/microsoft/vscode-eslint 插件
	// vscode中eslint的插件会使用eslint库，如果当前打开的项目没有安装eslint依赖，它会寻找全局依赖
	// 这个配置指示在使用全局eslint时用哪个包管理器找到这个eslint库
	...
	// 忽略警告
	"eslint.quiet": false,
	// 规定 ESlint 插件需要校验的语言类型，添加上对应的语言后 VSCode 才会在代码编辑器中正确的高亮错误的语法
	// 并且会在窗口中输出错误信息
	"eslint.probe": [
		"javascript",
		"javascriptreact",
